...
## 야담 원고 설계 요약 (story-engine yadam 장르)
- W1: 팩트체크(관직·신분·풍속 고증) + 자료수집(청구야담·어우야담·계서야담 등 원전, 변이형)
- W2: 20챕터 시놉시스(기1~5 / 승6~12 / 전13~17 / 결18~20) + 프리플라이트
  - 미끼 용의자 2명 이상, 반전 2중 이상, 진실 확신 시점은 15챕터 이후, 복선 추적표
- W3: 훅 → 기 → 승 → 전 → 결 순서로 쓰고 통합 검토(최대 5회, 목표 9.5점)
  - 분량 6,000~12,000자(약 18~36분), 비율 기15·승30·전35·결20%
  - "옛날 옛적"으로 시작, ~습니다/~지요 존댓말, 3인칭 관찰자, 고정 CTA로 마무리
  - 금지: 감정 직접 명명, 말줄임표, 외래어·영어, 아라비아 숫자, 그/그녀, ~씨, 하녀
  - Show don't tell, 정보는 조금씩, 해결 장면은 짧은 문장·동사 중심
  - 검토 1순위는 "다음 단락으로 끌고 가는가"(E0~E3), 문장 완성도(C1~C10)는 보조
- 원본 지침: touchizen/AutoFlowCut `skills/story-engine/meta-prompts/yadam/*.md`, `docs/ko/W1~W3`

## 폭발 영상 분석 결과 (2026-09, 사용자 제공 스크립트 3편: 떡집 할머니/씨름 소년/이조판서 시험)
- 오프닝: "옛날 옛적"이 아니라 이야기 중반 장면(대사 한 줄)으로 시작 → 비상식적 선택 + 구경꾼 비웃음
  → 시청자에게 직접 질문 1~2개 → 채널 인사·구독 요청 → "지금부터/옛날 옛적"으로 처음부터
- 두 엔진: 미스터리(시청자가 모름: "그 아이는 무엇을 알고?") 또는 극적 아이러니(시청자만 앎: 품팔이가 사실 판서)
- 공식: 약자의 작은 선행 반복 → 누군가(권력자·비밀 가진 자)가 지켜보고 있음 → 정체 공개 + 보상
- 분량 늘리는 단위: 시험/도전 에피소드 3회 이상 반복(도전자 연속 패배, 시험 누적), 각 단위 끝에 작은 훅
- 느린 생활 묘사(새벽 떡 찌기 등 오감)로 수면용 청취에 맞춤. 말투는 ~지요/~어요 혼용
- 대비 인물 쌍(좋은 며느리감 vs 나쁜 며느리감), 주인공 선행을 비추는 거울 인물(동생에게 떡 반쪽 남기는 아이)
- 원작 대사·문장은 가져오지 말고 구조만 참고할 것

## 전용 스킬: yadam-longform (70분·25,000자)
- 위치: `.claude/skills/yadam-longform/` — "야담 원고 써줘", "새 에피소드", 소재만 던져도 실행
- AutoFlowCut W1~W3 뼈대 + 장편화(30~36챕터) + 오프닝 6종·작법 6종 로테이션 + 반복 방지 기록장(`episodes/_log.md`)
- 틀을 고정하지 말 것 (사용자가 수익 정지 경험 있음 → 반복 콘텐츠 정책에 민감)
- 원고 출력: `episodes/ep{번호}_{제목}/`
